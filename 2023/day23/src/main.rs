...
fn get_next_steps(grid: &HashMap<Pt, char>, cur: &Pt, path: Vec<Pt>, use_slopes: bool) -> Vec<Pt> {
    // Check for steeps
    ...
    if use_slopes && (cur_c == '>' || cur_c == '<' || cur_c == '^' || cur_c == 'v') {
...
fn calc_hikes(grid: &HashMap<Pt, char>, start: &Pt, use_slopes: bool) -> Vec<Hike> {
    ...
    while hikes.len() > 0 {
        ...
        let next_steps = get_next_steps(grid, &h.cur, path.clone(), use_slopes);
        if next_steps.len() == 0 {
            let mut m_path = path;
            m_path.dedup();
            final_hikes.push(Hike {
                cur: h.cur,
                path: m_path,
            });
        } else {
...
fn _print_grid(grid: &HashMap<Pt, char>, path: Vec<Pt>, max: Pt) {
    for _y in 0..max.y + 1 {
        let y = _y as i32;
...
fn solve(grid: &HashMap<Pt, char>, end: &Pt, use_slopes: bool) -> usize {
    let mut max_path = 0;
    // let mut longest: Vec<Pt> = Vec::new();
    let hikes = calc_hikes(grid, &Pt { x: 1, y: 0 }, use_slopes);
    for h in hikes {
        let mut path = h.path.clone();
        path.dedup();
        if path.len() > max_path && h.cur == *end {
            max_path = path.len();
            // longest = path.clone();
        }
    }
...
fn main() {
    ...
    println!("Part 1: {}", solve(&grid, &end, true));
    let elapsed = now.elapsed();
    println!("Done in: {:.2?}!", elapsed);
    now = Instant::now();
    println!("Part 2: {}", solve(&grid, &end, false));
